...
try {
  const response = await api.post("/register-category",  {name: category.value });
  console.log(response);
  console.log("Categoria add com sucesso!")
  closeModal();
} catch (err) {
        ...
        <div
          ...
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              <div
                class="mx-auto flex size-12 shrink-0 items-center justify-center rounded-full bg-purple-100 sm:mx-0 sm:size-10">
                <svg class="size-6 text-purple-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
            <button @click="addNewCategory" type="button"
              class="cursor-pointer inline-flex w-full justify-center rounded-md bg-purple-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-purple-500 sm:ml-3 sm:w-auto">Salvar</button>
